...
if (isset($_POST['back'])) {
    header('location:display_Chapters.php?course='.$_GET['course'].'&chapter='.$_GET['chapter']);
    exit;
}

// Add document if add button is clicked
if (isset($_POST['add'])) {
    $name = $_POST['name'];
    ...
    if (empty($name) || empty($category) || empty($file)) {
        ...
        if (!in_array(strtolower($extension), $validTypes)) {
            ...
            if($chapterName != null){
            $chapterName->addDocument($document);
            $courseList->saveData();}
            header('location:display_documents.php?course='.$_GET['course'].'&chapter='.$_GET['chapter']);
            exit;
        }
    }
}
...
<form action="add_document.php?course=<?php echo $_GET['course']; ?>&chapter=<?php echo $_GET['chapter']; ?>" method="post" enctype="multipart/form-data">
...
<form action="display_documents.php?course=<?php echo $_GET['course']; ?>&chapter=<?php echo $_GET['chapter']; ?>" method="post">
